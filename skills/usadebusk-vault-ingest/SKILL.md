...
- A blank `## Task Durations` table is created with the card. It is ACTUALS ONLY — DSP/estimate hours are never written to it or restated as prose in Field Notes. Pre-job budget estimates from quotes/DSPs belong in `## Notes`.
...
- The structured table carries the numbers; Field Notes carries the "why." Never duplicate the duration numbers as prose.
- Field Notes entries require a real job number — pre-job estimates go in `## Notes` instead.
...
- **`## Task Durations` is actuals-only.** Never write DSP/estimated hours to it or restate duration numbers as prose.
