...
This skill manages the full lifecycle of a furnace decoking field job from setup through final report. It operates inside the USADeBusk Field Execution Claude Project — one thread per job. All shift data, extractions, and log entries accumulate in the thread conversation history and serve as the running job record. Claude reads back through the thread to find prior `/extract` outputs, `/log` entries, and `/setup` data when compiling status or reports.

**Workflow:**
`/setup` → job project created → shifts run → `/extract` + `/log` + `/email` each shift cycle → `/status` on demand → `/report` at demob

---

## Commands

### /setup — Job Project Initialization

**Trigger:** User types `/setup` or asks to set up a new job / start a job project.
...
**Output:** A structured job initialization block posted in the thread. This is the anchor record for the job — Claude reads it from thread history for all subsequent commands. Format:
...
### /extract — Service Receipt Extraction

**Trigger:** User types `/extract` or uploads scanned service receipts and asks for extraction, structured data, or a summary.
...
**Output format:** Read `references/extraction-format.md` for the exact structured block format. Output one block per receipt. Label clearly: DAY SHIFT — Ticket #NNNNN and NIGHT SHIFT — Ticket #NNNNN.

**After extraction:** Tell the user: "Review for any corrections, then type /email when ready to generate the email body."

---

### /log — Shift Note Append

**Trigger:** User types `/log` or sends a freeform note about anything that happened on the job — pig results, PDT events, equipment issues, observations, customer comments, stuck pig situations, anything.
...
- Flow test: pass #, before/after, RPM/PSI/GPM readings — structure into flow test table format for `/report`
...
### /email — Shift Email Body Generator

**Trigger:** User types `/email` after reviewing an /extract output, or asks to generate the shift email.
...
### /status — Job Progress Summary

**Trigger:** User types `/status` or asks where the job stands, how many hours burned, pig count, progress vs. quote.

**Inputs:** Read back through this conversation thread to find all prior `/extract` outputs and `/setup` data. Aggregate hours and pig counts from every extraction block in the thread.
...
### /report — Final Job Report Compiler

**Trigger:** User types `/report` or asks to generate / compile the job report.

**Inputs:** Read back through this conversation thread to find all prior `/extract` outputs, `/log` entries, `/setup` data, and any flow test data logged. Additionally prompt for: flow test data (if not already logged), pig photos (user attaches).
...
3. Assemble complete report structure (see `references/report-structure.md`)
4. Closing block uses PM name/cell/email from `/setup` data — not day supervisor
5. Call the `docx` skill to output a branded Word document

**User writes:** Decoking Summary / Analysis narrative only.
**Claude compiles:** Everything else from accumulated job data.
...
- **Flag hours overruns proactively** — if /extract data pushes any task category past 90% of quoted, note it in the extraction output without waiting for /status.
...
- `references/extraction-format.md` — exact output format for /extract, field definitions, edge case handling
- `references/report-structure.md` — complete job report section structure, table schemas, formatting rules
